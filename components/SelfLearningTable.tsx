...
import { Plus, Trash2, Calendar, AlignLeft, AlignCenter, AlignRight, Highlighter, MousePointer2, Minus, Layout, Square, Quote, Settings2, FileUp, Image as ImageIcon, Video, Music, FileText, Loader2, Wand2 } from 'lucide-react';
...
const SelfLearningTable: React.FC<SelfLearningTableProps> = ({ data, onUpdate }) => {
  ...
  const renderTopic = (topic: DPSSTopic, depth = 0) => (
    <div key={topic.id} style={{ marginLeft: `${depth * 15}px` }}>
      <div onClick={() => setSelectedTopicId(topic.id)} className={`p-2 my-1 rounded-lg cursor-pointer flex items-center justify-between ${selectedTopicId === topic.id ? 'bg-emerald-100/50' : 'bg-white/5 hover:bg-white/10'}`}>
    ...
    <div className="flex h-[90vh] p-2 gap-2">
      <div className="w-[300px] bg-white/10 backdrop-blur-md rounded-3xl p-4 border border-white/20 flex flex-col gap-4 overflow-hidden">
        <div className="flex items-center justify-between mb-4">
          ...
          <span className="text-[10px] bg-emerald-100 text-emerald-600 px-2 py-0.5 rounded-full font-bold uppercase">Personal Growth</span>
        </div>

        <button onClick={() => addTopic()} className="w-full py-3 bg-gradient-to-r from-emerald-500 to-orange-500 text-white rounded-xl text-xs font-black flex items-center justify-center gap-2 hover:from-emerald-600 hover:to-orange-600 shadow-lg shadow-emerald-200 transition-all">
        ...
        {selectedTopic ? (
            <div className="space-y-4 h-full flex flex-col">
                <input 
                    ...
                    className="w-full text-4xl font-black text-slate-900 bg-transparent outline-none p-2 border-b-2 border-emerald-500/20 focus:border-emerald-500 transition-all font-sans"
