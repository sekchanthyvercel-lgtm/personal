                       ...
                       <div className="flex items-center gap-3">
                          ...
                          <div>
                              ...
                              <h3 className="text-xl font-black text-slate-800 uppercase italic tracking-tighter flex items-center gap-3">
                                  ...
                                  <button onClick={() => { const html = `<ul style="list-style-type: none; padding-left: 0; margin-top: 4px; margin-bottom: 4px;"><li style="display: flex; gap: 8px; align-items: flex-start;"><span contenteditable="false" class="task-checkbox" style="cursor: pointer; user-select: none;">⬜</span><span>List item</span></li></ul><div><br></div>`; const dateKey = format(selectedPlanningDate, 'yyyy-MM-dd'); let safeVal = typeof planningNote === 'string' ? planningNote : ''; if (safeVal === '[object Object]') safeVal = ''; const newNote = safeVal + html; setPlanningNote(newNote); if (onUpdateDailyNote) { onUpdateDailyNote(dateKey, newNote); } else { onUpdate({ ...data, dailyNotes: { ...(data.dailyNotes || {}), [dateKey]: newNote } }); } }} className="text-slate-400 hover:text-emerald-600 transition-colors" title="Insert Checklist"><CheckSquare size={20} /></button>
                              </h3>
                          </div>
                       </div>
                    </div>
          ...
          <div className="lg:col-span-8 bg-white/10 backdrop-blur-2xl border border-white/20 rounded-[24px] p-6 shadow-xl relative overflow-hidden group">
              <div className="flex items-center justify-between mb-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h3 className="text-sm font-black text-slate-900 tracking-tighter uppercase italic flex items-center gap-2">Strategic Planning <button onClick={() => { const html = `<ul style="list-style-type: none; padding-left: 0; margin-top: 4px; margin-bottom: 4px;"><li style="display: flex; gap: 8px; align-items: flex-start;"><span contenteditable="false" class="task-checkbox" style="cursor: pointer; user-select: none;">⬜</span><span>List item</span></li></ul><div><br></div>`; const dateKey = format(selectedPlanningDate, 'yyyy-MM-dd'); let safeVal = typeof planningNote === 'string' ? planningNote : ''; if (safeVal === '[object Object]') safeVal = ''; const newNote = safeVal + html; setPlanningNote(newNote); if (onUpdateDailyNote) { onUpdateDailyNote(dateKey, newNote); } else { onUpdate({ ...data, dailyNotes: { ...(data.dailyNotes || {}), [dateKey]: newNote } }); } }} className="text-slate-400 hover:text-emerald-600 transition-colors" title="Insert Checklist"><CheckSquare size={16} /></button></h3>
